...
class UnityCameraVision(DroneBase):
    def __init__(self, video_port=5005, lidar_port=6006):
        ...
        self.output_path = os.path.join(
            OUTPUT_DIR, f"unity_record_{int(time.time())}.mp4"
        )
    ...
    def _init_video_writer(self, frame):
        ...
        self.video_writer = cv2.VideoWriter(self.output_path, fourcc, 30, (w, h))
        print(f"🎥 Запись видео начата: {self.output_path}")
    ...
    def process(self):
        ...
        # === Создаём окно и кнопку (если Qt доступен) ===
        cv2.namedWindow(WINDOW_CAMERA, cv2.WINDOW_NORMAL)

        def stop_program(*args):
            print("🛑 Кнопка нажата — завершаем работу...")
            self.running = False

        try:
            cv2.createButton("STOP PROGRAM", stop_program, None, cv2.QT_PUSH_BUTTON, 1)
            print("✔ Кнопка STOP PROGRAM создана")
        except:
            print("⚠ Qt-кнопки недоступны — используем клавишу Q для выхода")
        ...
        while self.running:
            ...
            if self.video_writer is None:
                self._init_video_writer(display_frame)
            ...
            if self.results is not None:
                ...
                for box in self.results.boxes:
                    ...
                    angle = int((center_x / w - 0.5) * CAMERA_FOV + 180)
            ...
            if key == ord('q'):
                print("🛑 Нажата Q — завершаем работу...")
                self.running = False
        ...
        if self.video_writer:
            ...
            print("💾 Видео сохранено:", self.output_path)
